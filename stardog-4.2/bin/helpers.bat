...
IF DEFINED STARDOG_HOME (
    SET extensions=yaml yml json jsn xml
    SET has_log4j=
    SET actual_ext=
    FOR %%E IN (%extensions%) DO (
        IF EXIST "%STARDOG_HOME%\log4j2.%%E" (
            SET has_log4j=true
            SET actual_ext=%%E
        )
    )
    IF "%has_log4j%"=="true" SET log4j_location=file:///%STARDOG_HOME%\log4j2.%actual_ext%
)
IF NOT DEFINED log4j_location SET log4j_location=file:///%STARDOG%\server\dbms\log4j2.xml
